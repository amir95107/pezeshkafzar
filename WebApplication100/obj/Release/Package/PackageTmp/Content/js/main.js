...
var loader = '<div class="text-center"><span class="slider-loader"></span></div>'
...
addToCart = (id) => {
    showPreloader();
    $.get("/Api/Shop/" + id, () => {
        $("#showCart , #showCart2,#showCart3").load("/ShopCart/ShowCart").fadeOut(200).fadeIn(200);
...
submitArchiveForm = (id) => {
    const min = $(id).find('#minPrice').val();
    const max = $(id).find('#maxPrice').val();
    if (max >= min) {
        $('#filterForm').submit();
    } else {
